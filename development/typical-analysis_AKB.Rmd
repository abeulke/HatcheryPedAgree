...
comp_filter
comp_1 <- comp_filter %>% separate(pa_unconstrained, c("PaUn_ID", "PaUn_Sex", "PaUn_Year", "PaUn_Date", "PaUn_Hatchery"), sep = "_") %>% separate(ma_unconstrained, c("MaUn_ID", "MaUn_Sex", "MaUn_Year", "MaUn_Date", "MaUn_Hatchery"), sep = "_")
comp_1

i <-  1:10437 #number of samples in comp_1
comp_2 <- comp_1 %>% mutate(Un_Same_Sex = ifelse(PaUn_Sex[i] == MaUn_Sex[i], yes = "Same Sex Parents", no ="NA"))
library(lubridate)
comp_3a <- comp_2 %>% mutate(PaUn_Date = mdy(PaUn_Date)) %>% mutate(MaUn_Date = mdy(MaUn_Date))
comp_3b <- comp_3a %>% mutate(Un_Diff_Spawn_Date = ifelse(PaUn_Date[i] != MaUn_Date[i], yes = "Different Spawn Dates", no = "Same"))
comp_4 <- comp_3b %>% mutate(Un_Diff_Hatchery = ifelse(PaUn_Hatchery[i] != MaUn_Hatchery[i], yes = "Parents from Different Hatcheries", no = "Same Hatchery"))
comp_5 <- comp_4 %>% mutate(Un_Diff_SpawnYear = ifelse(PaUn_Year[i] != MaUn_Year[i], yes = "Different Spawn Years", no = "Same Spawn Year"))
